...
1. 后续如需注释氨基酸变异, 修改 `test` 目录中的 `hbv_snp_to_aa.py` 脚本来实现. 来源: ChatGPT

## 更新

- \[20260112] 0.2.2
  - 修复 ivar trim 输入有信息头无条目输出无信息头空文件的 BUG
  - 修复 all\_vars\_summary.py 当所有样本都没有变异的情况, 会导致 depth\_df,freq\_df 为空, 后续合并会报错
- \[20251209] 0.2.1
...
- \[20251013] 根据浩博的修改意见修改流程和参数
  - 修改 fastp 参数 --cut\_front --cut\_tail --cut\_mean\_quality 20 --qualified\_quality\_phred 20 --length\_required 75 --detect\_adapter\_for\_pe --trim\_poly\_g
  - 新增 fastp 接头残留计算
